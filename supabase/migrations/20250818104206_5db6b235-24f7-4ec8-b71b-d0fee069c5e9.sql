...
CREATE POLICY "Users can view attachments for their tickets" ON public.support_ticket_attachments
FOR SELECT USING (
  ticket_id IN (
    SELECT id FROM public.support_tickets_v2 
    WHERE created_by = auth.uid() OR assigned_agent_id = auth.uid()
  ) OR is_admin()
);

CREATE POLICY "Users can upload attachments to their tickets" ON public.support_ticket_attachments
...
CREATE POLICY "Users can view SLA tracking for their tickets" ON public.support_sla_tracking
FOR SELECT USING (
  ticket_id IN (
    SELECT id FROM public.support_tickets_v2 
    WHERE created_by = auth.uid() OR assigned_agent_id = auth.uid()
...
-- 4. Fix database functions with missing search_path
...
-- 5. Create security events table for logging
