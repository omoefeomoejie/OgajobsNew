...
export const HeroSection = memo(() => {
  ...
  return (
    <section className="relative overflow-hidden">
      ...
      <div className="absolute inset-0 bg-gradient-to-br from-green-950 via-green-900 to-green-800 opacity-98"></div>
        ...
        <div className="max-w-4xl mx-auto text-center text-white">
          ...
          <div className="bg-white rounded-2xl p-6 md:p-8 shadow-2xl mb-12 max-w-3xl mx-auto">
            <div className="flex flex-col md:flex-row gap-4 mb-6">
              {/* Location Selector */}
              <div className="flex items-center gap-2 bg-muted px-4 py-3 rounded-lg md:w-64">
                <MapPin className="w-5 h-5 text-muted-foreground flex-shrink-0" />
                <select
                  value={selectedCity}
                  onChange={(e) => setSelectedCity(e.target.value)}
                  className="bg-transparent border-none outline-none text-foreground flex-1 text-sm cursor-pointer"
                >
                  <option value="">Where do you need help?</option>
                  {Object.entries(NIGERIA_STATES).sort().map(([state, areas]) => (
          ...
          <div className="flex flex-col md:flex-row gap-4 justify-center items-center">
            ...
            <Button size="lg" className="px-8 bg-orange-500 hover:bg-orange-600 text-white border-0" asChild>
